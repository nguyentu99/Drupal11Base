...
class CassiopeiaAdminAdministratorBlockItemDeleteForm extends FormBase {
  ...
  public function buildForm(array $form, FormStateInterface $form_state, $block = NULL) {
    ...
    $form['submit'] = array(
      ...
      '#suffix' => '&nbsp;&nbsp;' . Link::fromTextAndUrl(t('Cancel'), Url::fromUri('internal:/' . 'admin/cassiopeia/administrator/block/'.$block->id.'/items'), [])->toString()
    );
    return $form;
  }
  ...
  public function submitForm(array &$form, FormStateInterface $form_state) {
    ...
    if (!empty($block->id)) {
      ...
      \Drupal::messenger()
        ->addMessage(t('Deleted @name symbol', array('@name'=>$block->name)));
